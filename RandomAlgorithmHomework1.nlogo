...
to setup
  ...
  create-turtles number-of-nodes [
    set color one-of [ yellow green red blue]
    set size 2
    set label color
    set decided false
    set marked false
...
ask turtles[
  set thislabel label
  ask one-of turtles with [thislabel != label][
   create-link-with myself
  ]
]

;; if link already exists, nothing happens
...
;;procedure gia na ftiaxnoume to D+1 synolo xrwmatwn
to setup-colors
  let D max[count link-neighbors] of turtles

  set colors-list []
  let i 0
  while [i < D + 1][
    set colors-list fput random 140 colors-list
    set i i + 1]
...
to coloring-graph
 ...
 ; ask turtles with  [member? self (links-of myself)][

   ; set neighbors-color-list []
 ...
 ; ]
  ...
  ask turtles with [decided = false and marked = true][
      ...
      let b ([color] of link-neighbors with [decided = true])
      let temp-list colors-list
      show b

      foreach b [
       x -> set temp-list remove x temp-list
      ]
      set color one-of temp-list
      set label color
      set decided true
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [decided = true]"
